...
int main()
{
    ...
    vec angles = linspace(M_PI/3, M_PI, 100);
    vec r12s = linspace(1.0, 6.0, 10);
    vec r13s = linspace(1.0, 6.0, 100);
    struct fann *ann;
...
//    cout << "Input plus: " << setprecision(10) << input[0] << endl;
//    output = fann_run(ann, input);
//    double energyPlus = output[0];
//    cout << "Output plus: " << setprecision(10) << output[0] << endl;
//    fann_destroy(ann);
//    input[0] = rescale(r12 - h, r12s.min(), r12s.max());
//    cout << "Input minus: " << setprecision(10) << input[0] << endl;
//    ann = fann_create_from_file(inFileName.c_str());
//    output = fann_run(ann, input);
//    double energyMinus = output[0];
//    cout << "Output minus: " << setprecision(10) << output[0] << endl;
//    double derivative = (energyPlus - energyMinus) / (2*h);
//    cout << "Derivative: " << derivative << endl;
//    fann_destroy(ann);
//    input[0] = rescale(r12, r12s.min(), r12s.max());
//    ann = fann_create_from_file(inFileName.c_str());
//    output = fann_run_diff(ann, input);
//    cout << "First output: " << output[0] << endl;
//    fann_destroy(ann);
    ...
    for(int i = 0; i < int(r12s.n_elem); i++) {
        ...
        energiesOutFileName << "energies" << std::fixed << setprecision(4) << r12 << ".dat";
        stringstream derivativesOutFileName;
        derivativesOutFileName << "derivatives" << std::fixed << setprecision(4) << r12 << ".dat";
        ofstream energiesOutFile(energiesOutFileName.str());
        ofstream derivativesOutFile(derivativesOutFileName.str());
        fann_type *output;
...
//                cout << "Derivative: " << derivative << endl;
                ...
                derivativesOutFile << derivative << " ";
            }
            energiesOutFile << "\n";
            derivativesOutFile << "\n";
        }
        energiesOutFile.close();
        derivativesOutFile.close();
    }
    fann_destroy(ann);
    return 0;
}
